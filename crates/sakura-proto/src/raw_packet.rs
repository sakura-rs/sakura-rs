...
pub fn make_raw_packet(cmd_id: u16, head: crate::PacketHead, body: &[u8]) -> Box<[u8]> {
    ...
    (&mut buf[0..2]).copy_from_slice(&0x4567_u16.to_be_bytes());
    ...
    (&mut buf[10 + head_len..10 + head_len + body_len]).copy_from_slice(body);

    (&mut buf[10 + head_len + body_len..12 + head_len + body_len])
